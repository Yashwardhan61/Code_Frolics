// Maternal Family Tree JavaScript
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function saveMember() {
    try {
      ...
      // Save to Firebase
      await db.ref(`users/${currentUserSafeEmail}/maternalTree/${id}`).set(memberData);
  ...
  async function uploadPhoto(file, memberId) {
    ...
    const storageRef = storage.ref(`users/${currentUserSafeEmail}/maternalTree/${fileName}`);
  ...
  async function deleteMember() {
    try {
      ...
      await db.ref(`users/${currentUserSafeEmail}/maternalTree/${id}`).remove();
      
      // Delete descendants
      const descendants = findDescendants(id);
      for (const descendantId of descendants) {
        await db.ref(`users/${currentUserSafeEmail}/maternalTree/${descendantId}`).remove();
      }
  ...
  async function loadFamilyTree() {
    try {
      ...
      const snapshot = await db.ref(`users/${currentUserSafeEmail}/maternalTree`).once("value");
  ...
  function groupByGenerations(ancestors) {
    ...
    ancestors.forEach(member => {
      if (!currentRelation || 
          (currentRelation === "mother" && member.relationship === "grandmother") ||
          (currentRelation === "grandmother" && member.relationship === "great-grandmother")) {
        // New generation
  ...
  function openMemberDetails(member) {
    ...
    if (member.relativeTo) {
      ...
      if (parent) {
        relations.push({
          name: parent.name,
          relation: member.relationship === "self" ? "Mother" : 
  ...
  function openAddChildModal(parent) {
    ...
    if (parent.relationship === "self" || 
        ...
        parent.relationship === "great-grandmother") {
  ...
  function getRelationName(childRelation, parentRelation) {
    ...
    if (parentRelation === "self" && (childRelation === "brother" || childRelation === "sister")) {
      ...
      return "Niece";
    } else if (childRelation === "mother") {
      return "Mother";
    } else if (childRelation === "grandmother") {
      return "Grandmother";
    } else if (childRelation === "great-grandmother") {
  ...
  // Show loading state by default while auth is being verified
  loadingTree.classList.remove("hidden");
  familyTree.classList.add("hidden");
  emptyTree.classList.add("hidden");
